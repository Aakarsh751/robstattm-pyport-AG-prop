...
\begin{tabularx}{\linewidth}{@{} >{\bfseries}l X @{}}
  ...
  Project Repo     & \href{https://github.com/Aakarsh751/robstattm-pyport-AG}
                        {\texttt{github.com/Aakarsh751/robstattm-pyport-AG}} \\
...
\href{https://github.com/Aakarsh751/robstattm-pyport-AG/blob/main/robstattm/python/robstatpy\_comparison\_rpy2.ipynb}%
...
The following results demonstrate that the Python implementations match
the R originals.
All comparisons use the \texttt{flour} and \texttt{alcohol} datasets
from \texttt{RobStatTM}.

\subsection{Test 1: \texttt{locScaleM}, Bisquare and Huber Families}
...
\begin{tabularx}{\linewidth}{@{} l r r r c @{}}
  ...
  \midrule
  \multicolumn{5}{@{}l}{\textit{Bisquare, eff\,=\,0.95, alcohol data}} \\
  \quad $\hat{\mu}$    & 3.6970 & 3.6970 & $< 10^{-4}$ & PASS \\
  \quad std($\hat{\mu}$) & 0.2521 & 0.2521 & $< 10^{-3}$ & PASS \\
  \quad disper         & 0.8633 & 0.8633 & $< 10^{-4}$ & PASS \\[4pt]
  \multicolumn{5}{@{}l}{\textit{Huber, eff\,=\,0.95, alcohol data}} \\
  \quad $\hat{\mu}$    & 3.5828 & 3.5828 & $< 10^{-4}$ & PASS \\
  \quad std($\hat{\mu}$) & 0.2482 & 0.2482 & $< 10^{-3}$ & PASS \\
  \quad disper         & 0.8633 & 0.8633 & $< 10^{-4}$ & PASS \\
  \bottomrule
\end{tabularx}
\renewcommand{\arraystretch}{1}

\subsection{Test 2: \texttt{scaleM}, Bisquare and Huber Families}
...
\begin{tabularx}{\linewidth}{@{} l r r r c @{}}
  ...
  \textbf{Family} & \textbf{R value} & \textbf{Python value}
    & \textbf{$|\Delta|$} & \textbf{Status} \\
  \midrule
  Bisquare (flour data) & matches & matches & $< 10^{-4}$ & PASS \\
  Huber (flour data)    & matches & matches & $< 10^{-4}$ & PASS \\
  \bottomrule
\end{tabularx}
\renewcommand{\arraystretch}{1}
...
The comparison notebook also validates all five core functions by calling
R via \texttt{rpy2} and checking outputs:
...
\vspace{0.5em}
\noindent\textbf{Selected numerical outputs from the rpy2 wrapper:}

\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\linewidth}{@{} l l r @{}}
  \toprule
  \textbf{Function} & \textbf{Output Field} & \textbf{Value (via robstatpy)} \\
  \midrule
  \multicolumn{3}{@{}l}{\textit{lmrobdetMM (mineral, mopt, eff\,=\,0.95)}} \\
  & Intercept          & $15.2012$ \\
  & Slope (copper)     & $0.0126$ \\
  & Scale              & $9.9966$ \\
  & Robust $R^2$       & $0.0082$ \\[3pt]
  \multicolumn{3}{@{}l}{\textit{covRobMM (wine, 13 variables)}} \\
  & Center[1..5]       & $13.74,\; 2.01,\; 2.45,\; 17.04,\; 106.34$ \\
  & Max robust distance & $6.09$ \\[3pt]
  \multicolumn{3}{@{}l}{\textit{covRobRocke (wine, 13 variables)}} \\
  & Center[1..5]       & $13.77,\; 2.03,\; 2.44,\; 17.16,\; 107.52$ \\
  & Max robust distance & $5.79$ \\[3pt]
  \multicolumn{3}{@{}l}{\textit{pcaRobS (bus, 3 components, desprop\,=\,0.99)}} \\
  & propSPC[1]         & $> 0.80$ \\
  & propex             & $> 0.85$ \\
  \bottomrule
\end{tabularx}
...
\vspace{0.3em}
\noindent
All tests pass: 15/15 checks in the subprocess notebook and 14/14 in the
rpy2 notebook. Both notebooks are available in the project repository:
\href{https://github.com/Aakarsh751/robstattm-pyport-AG/blob/main/robstattm/python/robstatpy\_comparison.ipynb}{\texttt{robstatpy\_comparison.ipynb}} (subprocess) and
\href{https://github.com/Aakarsh751/robstattm-pyport-AG/blob/main/robstattm/python/robstatpy\_comparison\_rpy2.ipynb}{\texttt{robstatpy\_comparison\_rpy2.ipynb}} (rpy2).
...
project repository:
\href{https://github.com/Aakarsh751/robstattm-pyport-AG/blob/main/pcra/output/Ch2\_Comparison.pdf}{\texttt{pcra/output/Ch2\_Comparison.pdf}} (R vs.\ Python side-by-side).
